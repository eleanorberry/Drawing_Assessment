...
// List of buttons to be called in
let name_list = ["Rectangle", "Ellipse", "Triangle", "Heart", "Bouncing", "Rotating Triangle"];
let button_list = [];
...
for (let i = 0; i < name_list.length; i++) {
    button_list.push(new Button(x, y + i * (w / 3), w, h, name_list[i], colArray[1][0], colArray[1][9], colArray[1][10]));
}
Button.selected = button_list[0];
Button.Shape = button_list[0].text;

// Defining the line and pattern button
let Line_list = ["Line", "S", "M", "L"];
button_list.push(new Button(x, y + 528, w, h, Line_list[0], colArray[1][0], colArray[1][9], colArray[1][10]));
// Loop to make the options for line sizes
for (let i = 0; i < (Line_list.length - 1); i++) {
    button_list.push(new Size_button(x + i * 66.66, (y + 528) + h, 66.6, h, Line_list[i + 1], colArray[1][0], colArray[1][9], colArray[1][10]));
}
...
// loop to make the flower petal options
...
let undo = new Edit_button(750, 560, 115, 50, "Undo", c1, colArray[1][13], colArray[1][13]);
let redo = new Edit_button(750, 622, 115, 50, "Redo", c1, colArray[1][11], colArray[1][11]);
let clear = new Edit_button(875, 560, 90, 112, "Clear", c1, colArray[1][12], colArray[1][12]);
let C = new ControlObject(canvas);
// Calls in all the buttons and animates them
function animate() {
    ctx.clearRect(0, 0, width, height);
    clear.update();
    redo.update();
    undo.update();
    // loops to update the colour swatches
    for (let i = 0; i < colourSet.length; i++) {
        colourSet[i].update();
    }
    // loop to instantiate the button list
